...
class RecurrentNeuralBeamformer(
    torch.nn.Module
):  # jit.ScriptModule):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        self.selector_rate = selector_rate
    ...
    def forward_train(
        self, Y_STFT, init_phi_X=0.0, init_phi_Y=0.0
    ):
        ...
        for t in range(T):
            # print(t, T)
            Y_t = Y[:, :, :, t]  # N, 2, C, 1, F
            X_nn_t = X_nn_t.unsqueeze(3)
            W = self.update_beamformer_weight(t, X_nn_t, init_phi_X, inv_phi_Y)
